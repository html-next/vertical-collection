...
import { warn, assert, debugOnError } from 'vertical-collection/-debug/helpers';
...
export default class Geography {
  ...
  setState(state) {
    if (!state) {
      if (!this.element) {
        debugOnError(`Geography.setState() called without a state and without an element.`, false, {
          id: `vertical-collection.geography:potentially-invalid-set-state`
        });
        return;
      }
      state = this.element.getBoundingClientRect();
      debugOnError(`Geography.setState() performed a new measurement, but the result has no height.`, state.height > 0);
      assert(`Geography.setState() performed a new measurement, but the result has no width`, state.width > 0);
    }
