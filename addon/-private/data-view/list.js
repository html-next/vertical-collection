...
import makeDict from 'vertical-collection/-private/ember/make-dict';
...
export default class KeyList {
  ...
  get heightAbove() {
    ...
    for (i = 0; i < _firstItemIndex; i++) {
      debugOnError(`Proxy has no height`, ordered[i].height > 0);
      h += ordered[i].height;
    }
  ...
  slice(start, end) {
    ...
    start = start || start === 0 ? start : this.firstActiveIndex;
    end = end || start + _activeCount;

    assert(`list.slice: <end> cannot be less than zero`, end >= 0);
    assert(`list.slice: <end> cannot exceed the bounds of the array`, !length || end <= length);
