...
test('We can add, trigger, and remove a scroll handler', (assert) => {
  ...
  assert.notStrictEqual(scrollableIndex, -1, `The scrollable was added to the watched elements list.`);
...
test('Adding/removing multiple handlers to an element works as expected', (assert) => {
  ...
  afterNextScrollUpdate(() => {
    ...
    afterNextScrollUpdate(() => {
      ...
      assert.strictEqual(cache.handlers.length, 1, `The handler was removed from the listener cache.`);
      assert.notStrictEqual(newScrollableIndex, -1, `When an element has other handlers, it is not removed from the watched elements list.`);
      assert.notStrictEqual(scrollHandlers.handlers.indexOf(cache), -1, `When an element has other handlers, ths cache is not removed.`);
      assert.strictEqual(scrollHandlers.length, 1, `We have one element to watch.`);
...
test('Multiple elements with handlers works as expected', (assert) => {
  ...
  assert.notStrictEqual(scrollable1Index, -1, `The scrollable was added to the watched elements list.`);
  assert.notStrictEqual(scrollable2Index, -1, `The scrollable was added to the watched elements list.`);
  ...
  afterNextScrollUpdate(() => {
    ...
    afterNextScrollUpdate(() => {
      ...
      assert.strictEqual(cache1.handlers.length, 0, `The handler was removed from the listener cache.`);
      assert.strictEqual(newScrollableIndex, -1, `The element was removed from the watched elements list.`);
      assert.strictEqual(scrollHandlers.handlers.indexOf(cache1), -1, `The cache was also removed.`);
      ...
      assert.strictEqual(cache2.handlers.length, 0, `The handler was removed from the listener cache.`);
      assert.strictEqual(newScrollableIndex, -1, `Removing the last handler removed the element from the watched elements list.`);
      assert.strictEqual(scrollHandlers.handlers.indexOf(cache2), -1, `Removing the last handler removed the cache.`);
