...
module('Acceptance | Record Array', function(hooks) {
  ...
  test('RecordArrays render correctly', async function(assert) {
    ...
    assert.strictEqual(find('number-slide:first-of-type').textContent.replace(/\s/g, ''), '0(0)', 'correct first item rendered');
    assert.strictEqual(find('number-slide:last-of-type').textContent.replace(/\s/g, ''), '14(14)', 'correct last item rendered');
  });
  ...
  test('RecordArrays updates correctly after deleting items', async function(assert) {
    ...
    assert.strictEqual(findAll('number-slide').length, 5, 'correct number of items rendered and nothing crashes');
    assert.deepEqual(findAll('number-slide').map(s => s.textContent.trim()[0]), ['0', '1', '2', '3', '4'], 'correct items order');
  });
  ...
  test('RecordArrays fires firstVisibleChanged correctly after scrolling and fast-switching items', async function(assert) {
    ...
    assert.strictEqual(find('#first-visible-id').value, '0', 'the first item is the first visible id');

    await click('#last-25-button');
    assert.strictEqual(find('#first-visible-id').value, '75', 'the first visible id is updated correctly after updating items');

    await scrollTo('.table-wrapper', 0, 1000);
    assert.strictEqual(find('#first-visible-id').value, '86', 'the first visible id is updated correctly after scrolling');
    ...
    assert.strictEqual(find('#first-visible-id').value, '86', 'the first visible id is the same after fast-switching items');

    await scrollTo('.table-wrapper', 0, 0);
    assert.strictEqual(find('#first-visible-id').value, '75', 'the first visible id is updated correctly after scrolling');
  });
});
